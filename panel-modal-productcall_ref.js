; (function ($) {
  ...
  function api_Products() {
    ...
    var Payload = { "Brand": BRAND, "url": decodeURI(document.location.href).split('?')[0], "CONFIG": "on" }
  ...
  function showTryonReportPanel(TRcontent) {
    ...
    if ("ontouchend" in window) {
      $(".filter-button--panel").on("touchend", function () {
  ...
  function svg_display_panel(json_res) {
    function resize_svg_panel(data, Labels) {
      ...
      for (var l = 0; l < Labels.length; l++) {
        ...
        if (
          ...
          svg_data =
            svg_data +
            `<rect x=${avg_x * squaresize - 0.5 * rect_width > 0
              ? avg_x * squaresize - 0.5 * rect_width
              : 1
            } y=${avg_y * squaresize - 0.5 * rect_height
            } width=${rect_width} height=${rect_height} rx="10" ry="10" fill="white" stroke="rgb(27, 27, 27)" stroke-width="1"/><text letter-spacing="1px" x=${avg_x * squaresize + x_offset
            } y=${avg_y * squaresize
            } text-anchor="middle" dominant-baseline="central"  fill="rgb(27, 27, 27)" font-size="12">${data[Labels[l]].linename
            } ${[
          ...
          svg_data =
            svg_data +
            `<rect x=${avg_x * squaresize - 0.5 * rect_width} y=${avg_y * squaresize - 0.5 * rect_height
            } width=${rect_width} height=${rect_height} rx="10" ry="10" fill="white" stroke="rgb(27, 27, 27)" stroke-width="1"/><text letter-spacing="1px" x=${avg_x * squaresize
            } y=${avg_y * squaresize
            } text-anchor="middle" dominant-baseline="central"  fill="rgb(27, 27, 27)" font-size="12">${data[Labels[l]].linename
            } ${[
              "value" in data[Labels[l]] ? data[Labels[l]].value : "",
          ...
          svg_data =
            svg_data +
            `<rect x=${avg_x * squaresize - 0.5 * rect_width} y=${y1 * squaresize - 0.5 * rect_height
            } width=${rect_width} height=${rect_height} rx="10" ry="10" fill="white" stroke="rgb(27, 27, 27)" stroke-width="1"/><text letter-spacing="1px" x=${avg_x * squaresize
            } y=${y1 * squaresize
            } text-anchor="middle" dominant-baseline="central"  fill="rgb(27, 27, 27)" font-size="12">${data[Labels[l]].linename
            } ${[
    ...
    $(document).ready(function () {
      ...
      $(document).on("click", ".size-btn--panel", function () {
        ...
        $("#svg_size--panel").find("span").text(size_active);
        console.log($("#svg_size--panel").find("span").text(size_active), " $('#svg_size--panel').find('span').text(size_active);");
